...
interface HistoryCatalogSendItemProps {
  title: string
  date: string
  time: string
  stage: string
...
export function HistoryCatalogSendItem({
  title,
  date,
  time,
  stage,
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500">
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <span className={`text-xs ${status ? 'text-cyan-500' : 'text-purple-600'}`}>
                {status || assignee}
              </span>
            </div>
          </div>
